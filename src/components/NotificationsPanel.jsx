...
export default function NotificationsPanel({ onClose, onRead, onNavigate }) {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([getMyNotifications(30), getReceivedRequests()]).then(([{ notifications: list }, requests]) => {
      if (cancelled) return;
      ...
      const merged = [...(list ?? []), ...requestNotifs].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
